...
async function updateAuctionStatuses() {
  ...
  await db.update(auctionsTable)
    .set({ status: "live" })
    .where(and(
      eq(auctionsTable.status, "scheduled"),
      sql`${auctionsTable.startTime} <= ${now}`
    ));
  const ended = await db.update(auctionsTable)
    .set({ status: "ended" })
    .where(and(
      eq(auctionsTable.status, "live"),
      sql`${auctionsTable.endTime} <= ${now}`
    ))
    .returning();
...
router.get("/", optionalAuth, async (req: AuthRequest, res) => {
  try {
    ...
    res.json({
      auctions: auctions.map(a => auctionWithSeller(a, sellerMap[a.sellerId])),
      total: count,
      page: pageNum,
...
router.post("/", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [seller] = await db.select({ id: usersTable.id, name: usersTable.name, avatar: usersTable.avatar })
...
router.get("/:id", optionalAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [seller] = await db.select({ id: usersTable.id, name: usersTable.name, avatar: usersTable.avatar })
      .from(usersTable).where(eq(usersTable.id, auction.sellerId));
    const recentBids = await db.select({
      ...
      .where(eq(bidsTable.auctionId, req.params.id))
      .orderBy(desc(bidsTable.amount))
      .limit(10);
...
router.patch("/:id", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [seller] = await db.select({ id: usersTable.id, name: usersTable.name, avatar: usersTable.avatar })
      .from(usersTable).where(eq(usersTable.id, updated.sellerId));
...
router.post("/:id/bids", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (minsLeft < 2) {
      await db.update(auctionsTable).set({ endTime: new Date(now.getTime() + 2 * 60 * 1000) })
        .where(eq(auctionsTable.id, auction.id));
    }

    // Notify previous highest bidder
    const [prevTopBid] = await db.select().from(bidsTable)
      .where(eq(bidsTable.auctionId, auction.id))
      .orderBy(desc(bidsTable.amount))
      .limit(1);
    ...
    await db.update(auctionsTable)
      .set({ currentPrice: amount, bidCount: sql`${auctionsTable.bidCount} + 1` })
      .where(eq(auctionsTable.id, auction.id));
    ...
    if (prevTopBid && prevTopBid.bidderId !== req.userId) {
      await db.insert(notificationsTable).values({
        ...
        message: `Someone placed a higher bid of $${amount.toFixed(2)} on "${auction.title}"`,
        auctionId: auction.id,
      });
    }
    ...
    await db.insert(notificationsTable).values({
      ...
      message: `New bid of $${amount.toFixed(2)} on "${auction.title}"`,
      auctionId: auction.id,
    });

    const [bidder] = await db.select({ name: usersTable.name, avatar: usersTable.avatar })
      .from(usersTable).where(eq(usersTable.id, req.userId!));
